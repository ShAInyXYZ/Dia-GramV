 ...
 * Ported from OtiaFlow (same author, unchanged geometry), moved into core so
 * the SVG snapshot routes wires exactly the way the canvas does. Without this
 * an export of a "routed" diagram came out with straight lines through the
 * cards, which is not a picture of what is on screen.
 *
 * headings: E(+x) W(-x) S(+y) N(-y)
 * Pure geometry — no DOM, no framework.
 */
// ── Orthogonal edge router — direction-aware A* around node obstacles ────────
...
export const DIR = { E: 0, W: 1, S: 2, N: 3 };
const OPP = [1, 0, 3, 2];
...
function isInsideAny(p, obstacles) {
    for (const r of obstacles)
...
function buildAxis(obstacleVals, keepVals) {
 ...
 */
export function routeOrthogonal(source, target, obstacles, exitDir = DIR.E, arriveDir = DIR.E) {
    const keepX = [
        source.x, target.x,
    ...
    const nearest = (arr, v) => {
        ...
        for (let i = 1; i < arr.length; i++)
            if (Math.abs(arr[i] - v) < Math.abs(arr[b] - v))
                b = i;
        return b;
    };
    ...
    const skey = (ix, iy, d) => (((iy * W + ix) << 2) | d) >>> 0;
    const h = (ix, iy) => Math.abs(X[ix] - X[gx]) + Math.abs(Y[iy] - Y[gy]);
    const g = new Map();
    const came = new Map();
    ...
    while (open.length && guard++ < 30000) {
        ...
        for (const [nix, niy, nd] of moves) {
            if (nix < 0 || niy < 0 || nix >= W || niy >= Y.length)
                continue;
            if (nd === OPP[cur.d])
                continue; // NO 180° reversals — kills socket curls
            ...
            if (!isEndpointCell && isInsideAny(b, obstacles))
                continue;
...
export function toBeveledPath(pts, radius = 14) {
    if (pts.length < 2)
        return '';
    ...
    const sgn = (v) => (Math.abs(v) < 1.5 ? 0 : Math.sign(v));
