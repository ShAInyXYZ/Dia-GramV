<script lang="ts">
  ...
  const path = $derived.by(() => {
    ...
    switch (shape) {
      ...
      default: return `M${r},0 H${W - r} Q${W},0 ${W},${r} V${H - r} Q${W},${H} ${W - r},${H} H${r} Q0,${H} 0,${H - r} Z`;
    }
  });
...
<svg class="shape" width={w} height={h} viewBox="0 0 {w} {h}" aria-hidden="true">
  ...
  {#if shape === 'window'}<line x1="0" y1="20" x2={w} y2="20" stroke={color} stroke-width="1" opacity=".55" /><circle cx="11" cy="10" r="2.5" fill={color} opacity=".8" /><circle cx="20" cy="10" r="2.5" fill={color} opacity=".5" />{/if}
  {#if shape === 'terminal'}<text x="10" y="15" font-family="var(--mono)" font-size="11" fill={color} opacity=".9">›_</text>{/if}
  {#if shape === 'device'}<rect x={w / 2 - 14} y={h - 6} width="28" height="3" rx="1.5" fill={color} opacity=".6" />{/if}
</svg>
