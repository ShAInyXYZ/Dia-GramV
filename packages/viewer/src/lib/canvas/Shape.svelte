<script lang="ts">
  ...
  // every constant below is designed for a 260px card; at swatch size scale them
  // with the box, otherwise a 9px dome on a 16px-high cylinder is an hourglass
  ...
  const path = $derived.by(() => {
    ...
    switch (shape) {
      ...
      case 'tab': { const t = 9 * k, tw = Math.min(90 * k, W * .4); return `M0,${r} Q0,0 ${r},0 H${tw} l${10 * k},${t} H${W - r} Q${W},${t} ${W},${t + r} V${H - r} Q${W},${H} ${W - r},${H} H${r} Q0,${H} 0,${H - r} Z`; }
      case 'folder': { const t = 8 * k, tw = Math.min(70 * k, W * .3); return `M0,${r + t} Q0,${t} ${r},${t} H${tw} l${t},-${t} H${W - r} Q${W},0 ${W},${r} V${H - r} Q${W},${H} ${W - r},${H} H${r} Q0,${H} 0,${H - r} Z`; }
...
<svg class="shape" width={w} height={h} viewBox="0 0 {w} {h}" aria-hidden="true">
  ...
  {#if shape === 'cylinder'}<path d="M0,{9 * k} A{w / 2},{9 * k} 0 0 0 {w},{9 * k}" fill="none" stroke={color} stroke-width="1.5" opacity={dim ? .5 : 1} />{/if}
  {#if shape === 'window'}
