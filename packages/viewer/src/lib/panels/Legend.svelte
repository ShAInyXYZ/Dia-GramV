...
<div class="legend" class:closed={!ui.legendOpen}>
  <button class="tog" onclick={() => (ui.legendOpen = !ui.legendOpen)}>{ui.legendOpen ? '▾ legend' : '▸ legend'}</button>
  {#if ui.legendOpen}
    <div class="items">
...
<style>
  .legend { position: absolute; left: 12px; bottom: 12px; z-index: 5; max-width: 420px; background: var(--s1); border: 1px solid var(--hair); border-radius: 7px; padding: 4px 8px 6px; font-family: var(--mono); font-size: 10px; color: var(--muted); }
  .legend.closed { padding: 2px 6px; }
  .tog { background: transparent; border: none; padding: 2px 0; font-size: 10px; color: var(--dim); }
  .items { display: flex; flex-wrap: wrap; gap: 3px 10px; margin-top: 2px; }
